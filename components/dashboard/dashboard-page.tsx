...
import { SignOut, Plus, Notebook, MagnifyingGlass, UserCircle } from "@phosphor-icons/react";
                    ...
                    <div className="flex justify-between h-16 items-center">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 bg-zinc-900 dark:bg-zinc-100 rounded-lg flex items-center justify-center">
                                <Notebook weight="fill" className="text-white dark:text-zinc-900" size={18} />
                            </div>
                            <h1 className="font-bold text-lg hidden sm:block tracking-tight text-black dark:text-white">Quick Note</h1>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex flex-col items-end text-sm mr-2">
                                <span className="font-semibold text-zinc-900 dark:text-white">{user?.name || 'User'}</span>
                                <span className="text-xs text-zinc-500 truncate w-32 text-right">{user?.email}</span>
                            </div>
                            <button
                                onClick={signout}
                ...
                <div className="max-w-4xl mx-auto">
                    <div className="flex items-center justify-between mb-12">
                        <div className="flex flex-col gap-1">
                            <h2 className="text-4xl font-extrabold tracking-tight text-zinc-900 dark:text-white">Your Notes</h2>
                            <p className="text-zinc-500 dark:text-zinc-400">Capture your thoughts, ideas, and tasks.</p>
                        </div>
                    ...
                    <div className="flex flex-col items-center justify-center min-h-[400px] border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-[32px] bg-white dark:bg-[#09090b] shadow-sm animate-pulse-slow">
                        <div className="w-20 h-20 bg-zinc-50 dark:bg-zinc-900 rounded-3xl flex items-center justify-center mb-6 shadow-inner">
                            <Notebook size={40} className="text-zinc-300 dark:text-zinc-700" />
                        </div>
                        <h3 className="text-2xl font-bold text-zinc-900 dark:text-white mb-2">No notes found</h3>
                        <p className="text-zinc-500 dark:text-zinc-400 max-w-xs text-center mb-8">
                            Looks empty here! Start your creative journey by creating your first note today.
                        </p>
